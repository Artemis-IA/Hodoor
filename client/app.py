...
def login():
    ...
    if request.method == "POST":
        ...
        response = requests.post(
            f"{SERVEUR_URL}/login", files={"file": ("image.jpg", image)}
        )
...
def recognize_face():
    ...
    if request.method == "POST":
        ...
        response = requests.post(
            f"{SERVEUR_URL}/recognize-face", files={"file": file_tuple}, headers=headers
        )
        if response.status_code == 200:
            return response.json()
        else:
...
def find_face():
    ...
    if request.method == "POST":
        ...
        file_tuple = (file.filename, file.read(), file.content_type)
        response = requests.post(
            f"{SERVEUR_URL}/find-face", files={"file": file_tuple}, headers=headers
        )
        if response.status_code == 200:
            return response.json()
        else:
...
def detect_age():
    ...
    if request.method == "POST":
        ...
        file_tuple = (file.filename, file.read(), file.content_type)
        response = requests.post(
            f"{SERVEUR_URL}/detect-age", files={"file": file_tuple}, headers=headers
        )
        if response.status_code == 200:
            return response.json()
        else:
...
def detect_gender():
    ...
    if request.method == "POST":
        ...
        response = requests.post(
            f"{SERVEUR_URL}/detect-gender", files=[file_tuple], headers=headers
        )
        if response.status_code == 200:
            return response.json()
        else:
...
def analyze_race():
    ...
    if request.method == "POST":
        ...
        file_tuple = ("file", file, file.content_type)
        response = requests.post(
            f"{SERVEUR_URL}/analyze-race", files=[file_tuple], headers=headers
        )
        if response.status_code == 200:
            return response.json()
        else:
